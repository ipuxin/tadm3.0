...
class Coupon_model extends MY_Model {
	...
	function getCouponList($arr,$order=array('CreatDate','DESC'),$limit=array()){
		$arr['IsHide!'] = 1;
	...
	function couponSend($UserId,$CouponId,$CouponNum){
		$coupon = $this->getCoupon($CouponId);
		if(!$coupon['CouponName'] || !$coupon['CouponAmount']){
			$Data['Num'] = 0;
			return $Data;
		}
		$time = time();
		...
		for($i=0;$i<$CouponNum;$i++){
			if($this->usercoupon_model->addCoupon($arr)){
				$getNum += 1;
			}
		}
		$Data['Num'] = $getNum;
	...
	function resetCoupon($arr){
		...
		$arr['CityNameShow'] = $arr['CityName'];
		if(!$arr['CityName'])$arr['CityNameShow'] = '不限';
	...
	function getCoupon($arr){
		if(is_array($arr))$arr['IsHide'] = 'null';
