...
<body>
	<section>
		<div class="title"><span>编辑商品</span></div>
		<div class="tab-box" style="margin-bottom:12px;">
			...
			<span class="zhanshi">展示城市</span>
        </div>
		<form class="main" action="/product/updProduct">
			<input type='hidden' name='id' value='<?=$info['id']?>'>
			<tabarea>
			<line>
				<span class="title">商品名称：</span>
				<input data-valids="required" class="long valid" name="ProductName" type="text" value="<?=$info['ProductName']?>">
			</line>
			<line>
			...
			<line>
				...
				<script>
					...
					var Category1 = '<?=$info["Category1"]?>';
					var Category2 = '<?=$info["Category2"]?>';
					var needSelCate2 = 1;
			...
			<line style="height:52px;">
				<input type="hidden" id="ImageMinBox" name="ImageMin" value='<?=$info['ImageMin']?>'>
				<span class="title">商品小图：</span>
				<span><img id="ImageMinImg" class="min" src="<?=$info['ImageMin']?>"></span>
				<label for="ImageMinUpload"><div class="btn">上传</div></label>
				<span style="margin-top:12px;">（32KB以内，高160*160宽）</span>
			</line>
			<line style="height:52px;">
				<input type="hidden" id="ImageBigBox" name="ImageBig" value='<?=$info['ImageBig']?>'>
				<span class="title">商品大图：</span>
				<span><img id="ImageBigImg" class="big" src="<?=$info['ImageBig']?>"></span>
			...
			<line style="height:52px;">
				...
				<span class="imgList">
					<?if($info['ImageList'])foreach($info['ImageList'] as $v){?>
					<ar><a target="_blank" href="<?=$v?>"><img class="big" src="<?=$v?>"><input type="hidden" value="<?=$v?>" name="ImageList[]"></a><i>x</i></ar>
					<?}?>
				</span>
			...
			<line>
				...
				<input data-valids="number" class="short valid" name="StorageCount" type="text" value="<?=$info['StorageCount']?>"><span>份</span>
			</line>
			<line>
				<span class="title">购买价格：</span>
				<input data-valids="bigzero number" class="short valid" id="Prices" name="Prices[Normal]" maxlength="6" type="text" value="<?=$info['Prices']['Normal']?>"><span>元</span>
				<span class="title">市场价格：</span>
				<input data-valids="bigzero number" class="short valid" name="Prices[Market]" maxlength="6" type="text" value="<?=$info['Prices']['Market']?>"><span>元</span>
			</line>
			<line>
				<span class="title">发货地址：</span>
				<input data-valids="required" class="short valid" name="DeliverAddress" type="text" value="<?=$info['DeliverAddress']?>">
			</line>
			<line>
				<span class="title">运费：</span>
				<input data-valids="number" class="short valid" id="freightAmout" name="freightAmout" type="text" value="<?=$info['freightAmout']?>"><span>元</span>
			</line>
			<line>
				<span class="title">商品描述：</span>
				<textarea data-valids="required" class="valid" name="Description"><?=$info['Description']?></textarea>
			</line>
			<line>
			...
			<line>
				...
				<input class="short valid" name="TeamMemberLimit" type="text" value="<?=$info['TeamMemberLimit']?>"><span>人</span>
			</line>
			<line>
				<span class="title">团购限时：</span>
				<input class="short valid" name="Alive" type="text" value="<?=$info['Alive']?>"><span>单位小时</span>
			</line>
			<line>
				<span class="title">抽奖人数：</span>
				<input class="short valid" id="LotteryCount" name="LotteryCount" type="text" value="<?=$info['LotteryCount']?>"><span>人 (若大于0，则开启抽奖功能/应小于开团人数)</span>
			</line>
			</tabarea>
			<tabarea>
			<line>
				...
				<textarea id="editor" name="Content" style="margin-left:90px;width:1024px;height:500px;"><?=$info['Content']?></textarea>
	...
	<script>
		...
		var ProductType = '<?=$info["ProductType"]?>';
