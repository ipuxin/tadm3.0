...
<body>
	<section>
		...
		<form action="/product/updProduct">
			...
			<line>
				...
				<span class="title">待支付数：</span>
				<span><?=$info['SalesCount']['Waiting']?></span>
				<span class="title">展示销量：</span>
				<span><?=$info['SalesCount']['Adjust']?></span>
			</line>
			<line>
			...
			<line>
				...
				<span><?=$info['IsForSaleMsg']?></span>
			</line>
			<line>
				<span class="title">审核：</span>
				<input type="radio" name="IsChecked" value="1" checked="checked"><span>通过</span>
				<input type="radio" name="IsChecked" value="2"><span>不通过</span>
			</line>
			</tabarea>
			<tabarea>
			<line>
	...
	<script>
		...
		var IsChecked = '<?=$info["IsChecked"]?>';
		$('input[name="IsChecked"]').attr("checked",false);
		$('input[name="IsChecked"][value="'+IsChecked+'"]').prop("checked",true);
