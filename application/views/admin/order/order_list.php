	...
	<section>
		...
		<form class="tab" action="/order/getOrderList">
            ...
            <div class="filter-box">
				...
				<line>
					<span>店铺名称：</span><input type="text" name="ShopName" class="short">
		...
		var pagejs = {
            ...
            datamodel: [
				...
				{value:function(mo){
					...
					btn.click(function(){
						self.location='/admin/order/order_info/'+$(this).data('id');
					});
					var btn2 = $("<button data-id='"+mo.TeamId+"'>拼团详情</button>");
					btn2.click(function(){
						self.location='/admin/team/team_info/'+$(this).data('id');
					});

					var btn3 = $("<button data-userid='"+mo.UserId+"'>推送优惠券</button>");
					btn3.click(function(){
						var userId = $(this).data('userid');
						window.open("/admin/coupon/coupon_send/"+userId,'newindow','height=300,width=350,top=400,left=800,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no');
					});
					var warp = $('<btns></btns>');
					warp.append(btn);
