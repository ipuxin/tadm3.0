...
<body class="bg-grey">
	<warpbox>
		<tit><span>编辑</span> 焦点图</tit>

		<form class="tab" action="/banner/updBanner">
			<input type="hidden" name="id" value="<?=$Banner['id']?>">
			<line style="height:180px;">
				<input type="hidden" id="ImageInput" name="Url" value='<?=$Banner['Url']?>'>
				<span class="title">图片上传：</span>
				<span><img id="Image" class="min" src="<?=$Banner['Url']?>"></span>
				<label for="ImageButton"><div class="btn">上传</div></label>
				<span>(150KB以内,宽640*320高)</span>
			</line>
			<line>
				<span class="title">图片名称：</span>
				<input type="text" data-valids="required" class="valid" name="Name" value="<?=$Banner['Name']?>">
			</line>
			<line>
				<span class="title">图片排序：</span>
				<input type="text" data-valids="number" class="valid" name="Paixu" value="<?=$Banner['Paixu']?>">
				<span>(数字越大越靠前)</span>
			</line>
			<line>
	...
	<script>
		...
		var IsDisable = '<?=$Banner["IsDisable"]?>';
