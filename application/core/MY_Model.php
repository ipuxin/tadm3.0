...
class MY_Model extends CI_Model {
	...
	function add($arr){
		...
		$this->putDate($arr);
		$log = 0;
		if($this->_api_table=='Order'){
			$log = 1;
		}
		$res = $this->getcurl('',$log);
		if($this->_return_code===0){
			return $this->_return_Result[0];
		}
		return false;
	...
	function getcurl($url=''){
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, $dataReal);
